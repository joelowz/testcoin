...
*Setup instructions for a Gitian build of Chncoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Chncoin
...
- [Building Chncoin](#building-chncoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-chncoin-host/README.md).
...
Clone the git repositories for chncoin and Gitian and then checkout the chncoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/chncoin/chncoin
cd chncoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the chncoin repository
...
To build Chncoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the chncoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/chncoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/chncoin/chncoin
...
For example:
```bash
URL=https://github.com/laanwj/chncoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit chncoin=${COMMIT} --url chncoin=${URL} ../chncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit chncoin=${COMMIT} --url chncoin=${URL} ../chncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit chncoin=${COMMIT} --url chncoin=${URL} ../chncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/chncoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/chncoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/chncoin-build.assert
```
...
[chncoin/gitian.sigs](https://github.com/chncoin/gitian.sigs/) repository, or if that's not possible create a pull
