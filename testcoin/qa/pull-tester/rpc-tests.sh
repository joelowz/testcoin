...
export CHNCOINCLI=${BUILDDIR}/qa/pull-tester/run-chncoin-cli
export CHNCOIND=${REAL_CHNCOIND}

#Run the tests

if [ "x${ENABLE_CHNCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and chncoind must all be enabled"
fi
