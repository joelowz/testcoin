...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars CHNCOIND and CHNCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

CHNCOIND=${CHNCOIND:-${1}/chncoind}
CLI=${CHNCOINCLI:-${1}/chncoin-cli}
...
$CHNCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$CHNCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
